...
class Preprocessing():
    ...
    def _normalization(self,tweet): 
        ...
        stemming = self._stemming(stopword)
    
        return [cleaning, casefolding, tokenizing, normalisasi, stopword, stemming]

    def callback(self, x):
        cleaning, casefolding, tokenizing, normalisasi, stopword, stemming = self._normalization(x)
